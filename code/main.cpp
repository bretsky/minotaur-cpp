...
static void drawShapes(cv::UMat &image, const std::vector<std::vector<cv::Point> > &squares) {
    for (const auto &square : squares) {
        cv::polylines(image, cv::InputArrayOfArrays(square), true, cv::Scalar(255, 0, 0), 1, cv::LINE_AA);
    }
}

// Displays text in the center of a contour
void addLabel(cv::UMat &im, const std::string &label, std::vector<cv::Point> &contour) {
...
static cv::UMat findShapes(
    ...
    for (int i = 0; i < contours.size(); i++) {
        ...
        if (approx.size() == 3 &&
            ...
            addLabel(dst, "TRI", contours[i]);    // Triangles
        ...
        //     //  addLabel(dst, "PENTA", contours[i]);
        //     // else if (vtc == 6 && mincos >= -0.55 && maxcos <= -0.45)
        //     //  addLabel(dst, "HEXA", contours[i]);
...
int main(int argc, char *argv[]) {
    ...
    const cv::UMat image = cv::imread("triangles.bmp", CV_LOAD_IMAGE_COLOR).getUMat(cv::ACCESS_READ);
    cv::UMat dst;
    std::vector<std::vector<cv::Point>> triangles;
    std::vector<std::vector<cv::Point>> rectangles;
    std::vector<std::vector<cv::Point>> circles;
    dst = findShapes(image, triangles, rectangles, circles);
    std::cout << triangles.size() << std::endl;
    drawShapes(dst, triangles);
